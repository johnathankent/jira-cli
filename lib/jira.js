#!/usr/bin/env node
(function() {
  var JiraHelper, addItem, addWorklog, args, argv, configFile, configFilePath, createConfigFile, dutils, fs, getProject, jiraCli, keytar, listProjects, loadConfigFile, paramIsText, path, transitionItem;
    ...
    return dutils.ask("Jira Host", /.+/, function(host) {
      return dutils.ask("Jira Port", /.+/, function(port) {
        return dutils.ask("Username", /.+/, function(username) {
          return dutils.ask("Password (stored in keychain)", /.+/, function(password) {
  ...
  if (require.main === module) {
    ...
    args = argv.argv;
    configFilePath = path.join(process.env.HOME, '.jiraclirc');
